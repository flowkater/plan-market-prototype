...
function* postPlanTemplateSaga(action) {
    try {
        ...
        const taskTemplateList = state.newTaskTemplateList.get(
            "taskTemplateList"
        );
        const data = {
            plan_template: {
        ...
        if (payload.data.plan_template) {
            yield put(
                newPlanTemplateActions.successPostPlanTemplate({
                    newPlanTemplateId: payload.data.plan_template.id
                })
            );
        }
    } catch (e) {
